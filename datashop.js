...
const UI = {
  ...
  showPanel(name) {
    ...
    if (name==='db') {
      // Pre-render all three tabs so they're ready when user switches between them
      this.renderSchema();
      this.renderDBTabs();
      this.renderCurrentTable();
      APP.renderERD();
      // Then activate schema tab (hides ERD + data, shows schema)
...
function explainSQL(sql) {
  ...
  if (sl.startsWith('select')) {
    ...
    // NOTE: HAVING already added above via havingM match — no duplicate needed here
...
(function initSynFilter() {
  ...
  function applyFilter(filter) {
    ...
    // Remove any existing "meer" banner
    ...
    const hidden = allCards.filter(card => {
      const cat = card.dataset.cat || '';
      return !(filter === 'all' || cat === filter);
    });

    // Hide non-matching cards
    hidden.forEach(card => card.classList.add('syn-hidden'));

    // Show max 2 matching, hide the rest + add "+X meer" banner
    matching.forEach((card, i) => {
      card.classList.remove('syn-hidden');
      if (i >= MAX_VISIBLE) card.classList.add('syn-hidden');
    });

    const overflow = matching.length - MAX_VISIBLE;
    if (overflow > 0) {
      const lastVisible = matching[MAX_VISIBLE - 1];
      const banner = document.createElement('div');
      banner.className = 'syn-meer-banner';
      banner.textContent = `+${overflow} meer…`;
      banner.title = 'Klik om alle kaarten te tonen';
      banner.style.cssText = 'cursor:pointer;text-align:center;padding:10px;color:var(--cyan);font-weight:700;font-size:14px;';
      banner.addEventListener('click', () => {
        matching.forEach(card => card.classList.remove('syn-hidden'));
        banner.remove();
      });
      lastVisible.insertAdjacentElement('afterend', banner);
    }
  }
