...
(function initSynFilter() {
  ...
  function applyFilter(filter) {
    const grid = document.querySelector('.syn-grid');
    if (!grid) return;

    // Remove existing show-more banner
    const existingBanner = grid.querySelector('.syn-show-more');
    if (existingBanner) existingBanner.remove();

    const cards = [...grid.querySelectorAll('.syn-card')];
    // Determine which cards match the current filter
    const matching = cards.filter(card => {
      const cat = card.dataset.cat || '';
      return filter === 'all' || cat === filter;
    });
    const nonMatching = cards.filter(card => {
      const cat = card.dataset.cat || '';
      return !(filter === 'all' || cat === filter);
    });

    // Hide non-matching cards
    nonMatching.forEach(card => card.classList.add('syn-hidden'));

    // Show matching cards, limit to MAX_VISIBLE initially
    matching.forEach((card, i) => {
      card.classList.remove('syn-hidden');
      if (i >= MAX_VISIBLE) card.classList.add('syn-hidden');
    });

    // Add "+X meer" banner if there are hidden matching cards
    const hidden = matching.length - MAX_VISIBLE;
    if (hidden > 0) {
      const banner = document.createElement('button');
      banner.className = 'syn-show-more';
      banner.textContent = `+ ${hidden} meer tonen`;
  ...
  function setup() {
    ...
    if (!bar || bar._synInit) return;
    bar._synInit = true;

    // Apply initial state (show first MAX_VISIBLE for active filter or 'all')
    const activeBtn = bar.querySelector('.syn-filter-btn.active');
