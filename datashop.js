...
// Hulpfunctie: probeert alle geldige SQL-varianten, slaagt als één slaagt
function multiCheck(sql, validators) {
...
const UI = {
  ...
  updateXP() {
    ...
    $('xp-to-next').textContent = next ? (next.min - G.xp) + ' XP → ' + next.title : '✦ MAX LEVEL ✦';
  ...
  renderScenarios() {
    ...
    $('prog-lbl').textContent   = done+'/'+total+' voltooid · '+pct+'%';
    ...
    if (!list.length) {
      EL['sc-list'].innerHTML = `<div class="sc-empty-state">
  ...
  showPanel(name) {
    ...
    if (nb) nb.classList.add('active');
      ...
      if (DEBUG_DB) {
        console.log('[DEBUG BUG1] showPanel(db) called');
        console.log('[DEBUG BUG1] erd-container exists:', !!document.getElementById('erd-container'));
        console.log('[DEBUG BUG1] db-tab-erd display before:', document.getElementById('db-tab-erd')?.style.display);
        console.log('[DEBUG BUG1] panel-db has on class:', document.getElementById('panel-db')?.classList.contains('on'));
      }
      APP.showDbTab('schema'); this.renderCurrentTable();
    }
...
const APP = {
  ...
  showDbTab(tab) {
    ['schema','erd','data'].forEach(t => {
      const el = $('db-tab-'+t);
      const btn = $('dbt-'+t);
      // Use 'block' (not '') so inline style overrides the CSS display:none on #db-tab-erd and #db-tab-data
  ...
  renderERD() {
    ...
    const DEBUG_DB = false;
    if (DEBUG_DB) {
      console.log('[DEBUG BUG1] renderERD called, erd-container:', !!c);
      console.log('[DEBUG BUG1] db-tab-erd display:', document.getElementById('db-tab-erd')?.style.display);
      console.log('[DEBUG BUG1] DB keys:', Object.keys(DB));
    }
    if (!c) { if (DEBUG_DB) console.error('[DEBUG BUG1] erd-container NOT FOUND - early return!'); return; }
    const relations = [
...
function explainSQL(sql) {
  ...
  if (sl.startsWith('select')) {
    ...
    // HAVING is already added above via havingM — no duplicate needed here
            ...
            <div class="tut-lesson-body">
              ...
              <div class="tut-exercise">
                ...
                <div class="tut-exercise-hint-wrap">
                  <button class="btn btn-outline btn-sm tut-hint-toggle" onclick="this.nextElementSibling.style.display=this.nextElementSibling.style.display==='none'||this.nextElementSibling.style.display===''?'block':'none';this.textContent=this.nextElementSibling.style.display==='block'?'🙈 Hint verbergen':'💡 Toon hint'">💡 Toon hint</button>
...
(function initSynFilter() {
  ...
  function applyFilter(filter) {
    // Remove any previous "+X meer" banner
    document.querySelectorAll('.syn-meer-banner').forEach(el => el.remove());

    const allCards = Array.from(document.querySelectorAll('.syn-card'));
    const matching = allCards.filter(c => filter === 'all' || (c.dataset.cat || '') === filter);
    const rest     = allCards.filter(c => !(filter === 'all' || (c.dataset.cat || '') === filter));

    rest.forEach(c => c.classList.add('syn-hidden'));
    matching.forEach((c, i) => {
      i < MAX_VISIBLE ? c.classList.remove('syn-hidden') : c.classList.add('syn-hidden');
    });

    const overflow = matching.length - MAX_VISIBLE;
    if (overflow > 0) {
      const banner = document.createElement('button');
      banner.className = 'syn-meer-banner';
      banner.textContent = `+${overflow} meer…`;
      banner.style.cssText = 'display:block;width:100%;margin-top:8px;padding:10px;background:none;border:1px dashed var(--border2);border-radius:8px;color:var(--cyan);font-weight:700;font-size:14px;cursor:pointer;';
      banner.addEventListener('click', () => {
        matching.forEach(c => c.classList.remove('syn-hidden'));
        banner.remove();
      });
      matching[MAX_VISIBLE - 1].insertAdjacentElement('afterend', banner);
    }
  }

  function setup() {
    const bar = document.querySelector('.syn-filter-bar');
    if (!bar || bar._synInited) return;
    bar._synInited = true;
    // Apply initial filter so init state also respects MAX_VISIBLE
    applyFilter('all');
...
// ── EVENT DELEGATION ─────────────────────────────────────────────
// Single listener replaces all onclick="APP.x()" inline handlers
document.addEventListener('click', function(e) {
  // loadExampleIdx delegation
  const dbEl = e.target.closest('[data-dbtab]');
  if (dbEl) { APP.showDbTab(dbEl.dataset.dbtab); return; }
